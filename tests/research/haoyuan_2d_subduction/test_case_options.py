...
def test_2d_mantle_jump():
    ...
    options in relation to the "1100" in the mow project
    '''
...
def test_2d_mantle_jump_1():
    '''
    test for including a middle-lower mantle viscosity jump
    options in relation to the "1100i" in the mow project
    '''
    source_dir = os.path.join(fixture_root, "test_2d_mantle_jump_1")
    json_path = os.path.join(source_dir, 'case0.json')
    output_dir = os.path.join(test_dir,'test_2d_mantle_jump_1')
    if os.path.isdir(output_dir):
        rmtree(output_dir)
    create_case_with_json(json_path, CASE_TWOD, CASE_OPT_TWOD)  # create case
    assert(os.path.isdir(output_dir))  # check case generation
