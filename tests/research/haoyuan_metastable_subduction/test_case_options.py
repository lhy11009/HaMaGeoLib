...
def test_viscosity_profile_middle_mantle_metastable_grain_size():
    '''
    test for including a middle mantle layer
    that have maximum rheology around 1100 km
    cartesian, 2-d consistent geometry
    ...
    '''
    source_dir = os.path.join(fixture_root, "test_viscosity_profile_middle_mantle_metastable_grain_size")
    json_path = os.path.join(source_dir, 'case0.json')

    # output directory
    output_dir = os.path.join(test_dir,'test_viscosity_profile_middle_mantle_metastable_grain_size')
    if os.path.isdir(output_dir):
        rmtree(output_dir)
...
def test_haoyuan_metastable_subduction_3d_update_refinement():
    '''
    test for including metastable_subduction
    cartesian, 3d geometry
    update refinement in the sp_lower composition
    '''
    source_dir = os.path.join(fixture_root, "eba2d_width80_h1000_bw4000_sw1000_yd300_3d_update_refinement")
    json_path = os.path.join(source_dir, 'case0.json')

    # output directory
    output_dir = os.path.join(test_dir,'eba2d_width80_h1000_bw4000_sw1000_yd300_3d_update_refinement')
    if os.path.isdir(output_dir):
        rmtree(output_dir)
...
# todo_mow
def test_haoyuan_metastable_subduction_3d_version_four():
    '''
    test for version 4. Important features are:
    New solver failure schemes; prescribed field plugins;
    New particle generators, etc.
    '''
    source_dir = os.path.join(fixture_root, "metastable_subduction_3d_version_four")
    json_path = os.path.join(source_dir, 'case0.json')

    # output directory
    output_dir = os.path.join(test_dir,'metastable_subduction_3d_version_four')
    if os.path.isdir(output_dir):
        rmtree(output_dir)
