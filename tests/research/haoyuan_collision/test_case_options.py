...
rules = [PostProcessorRule(), CasePathRule(), RemoveFluidRule(), CompositionRule(),
         GeometryRule(), SlabRule(), RheologyRule(), WeakLayerRule(),
         SolverRule(), KinematicDrivenRule(), PrescribConditionRule(), OceanRule(), ContinentRule(),
...
def test_kinematic_driven_continent_setup():
    ...
    # case_prm and case_wb - the prm and wb files generated
    # case_prm_standard and case_wb_standard - the standand output file to compare with
    case_dir = test_dir/"test_kinematic_driven_continent"
    fixture_dir = fixture_root/"test_kinematic_driven_continent"
    ...
    config = {"use_my_setup_of_postprocess": True,
              ...
              "stokes_solver_type": "block GMG", # solver
              ...
              "kinematic_driven_condition": True, # kinematic driven
              "kinematic_driven_condition_domain_modification": [3000e3, 2200e3], # kinematic drive, move the lateral position of boundaries
                "add_continents": "both", # continent
                "subducting_continent_length": 1000e3 # continent
              }
...
def test_continent_chapman_partition_coefficent():
    """
    Test configurations of continents with a partition-coefficent-based Chapman thermal model
    """
    # set up tests
    # case_dir - directory of the test case. Remove the old ones if existing previously
    # template_prm and template_wb - templates to use for the prm and wb file
    # case_prm and case_wb - the prm and wb files generated
    # case_prm_standard and case_wb_standard - the standand output file to compare with
    case_dir = test_dir/"test_continent_chapman_partition_coefficent"
    fixture_dir = fixture_root/"test_continent_chapman_partition_coefficent"
    ...
    config = {"use_my_setup_of_postprocess": True,
              ...
              "weak_layer_compositions": ["MORB", "sediment"], # weak layer, normal setting
              "force_weak_layer_max_refinement": True, # weak layer
              "plate_start_point_pre_modification": 0.0, # slab
              "plate_end_point_pre_modification": 1e8, # slab
              "slab_layer_compositions": ["sediment", "MORB", "gabbro"], # slab
              "slab_layer_depths": [0.0, 4e3, 7.5e3, 15e3], # slab & remove peridotite
              "stokes_solver_type": "block GMG", # solver
              "skip_expensive_stokes": True, # solver
              "max_nonlinear_iterations": 40, # solver
              "linear_solver_tolerance": 1e-4, # solver
              "number_of_cheap_Stokes_solver_steps": 60, # solver
              "GMRES_solver_restart_length": 100, # solver
              "kinematic_driven_condition": True, # kinematic driven
              "kinematic_driven_condition_domain_modification": [3000e3, 2200e3], # kinematic driven, move the lateral position of boundaries
              "kinematic_driven_condition_assign_velocity_component": "all", # kinematic driven, change conditions to prescribe on the boundary
              "convergence_rate": 0.05, # prescribed/kinematic-driven condition
                "add_continents": "both", # continent
                "subducting_continent_length": 1000e3, # continent
                "chapman_model_type": "partition coefficient", # continent
                "chapman_model_surface_heatflux": 0.04 # continent
              }
...
def test_fix_mantle_rheology():
    """
    Test fix mantle rheology after removing the fluid component
    """
    # set up tests
    # case_dir - directory of the test case. Remove the old ones if existing previously
    # template_prm and template_wb - templates to use for the prm and wb file
    # case_prm and case_wb - the prm and wb files generated
    # case_prm_standard and case_wb_standard - the standand output file to compare with
    case_dir = test_dir/"test_fix_mantle_rheology"
    fixture_dir = fixture_root/"test_fix_mantle_rheology"
...
def test_phase_transition_setup_0():
    """
    Test phase transition setup
    """
    # set up tests
    # case_dir - directory of the test case. Remove the old ones if existing previously
    # template_prm and template_wb - templates to use for the prm and wb file
    # case_prm and case_wb - the prm and wb files generated
    # case_prm_standard and case_wb_standard - the standand output file to compare with
    case_dir = test_dir/"test_phase_transition_setup_0"
    fixture_dir = fixture_root/"test_phase_transition_setup_0"
...
def test_phase_transition_setup_1():
    """
    Test phase transition setup
    """
    # set up tests
    ...
    # case_prm and case_wb - the prm and wb files generated
    # case_prm_standard and case_wb_standard - the standand output file to compare with
    case_dir = test_dir/"test_phase_transition_setup_1"
    fixture_dir = fixture_root/"test_phase_transition_setup_1"

    if case_dir.is_dir():
        shutil.rmtree(case_dir)
    case_dir.mkdir(exist_ok=False)

    template_prm = package_root/"hamageolib/research/haoyuan_collision0/files/05092026/post_compressible_test.prm"
    template_wb = package_root/"hamageolib/research/haoyuan_collision0/files/05092026/original.wb"
    template_json = fixture_dir/"current.json"
...
def test_rebased_initial_topography():
    """
    Test model setup with an intial topography and on a rebased aspect branch
    """
    # set up tests
    # case_dir - directory of the test case. Remove the old ones if existing previously
    # template_prm and template_wb - templates to use for the prm and wb file
    # case_prm and case_wb - the prm and wb files generated
    # case_prm_standard and case_wb_standard - the standand output file to compare with
    case_dir = test_dir/"test_rebased_initial_topography"
    fixture_dir = fixture_root/"test_rebased_initial_topography"

    if case_dir.is_dir():
        shutil.rmtree(case_dir)
    case_dir.mkdir(exist_ok=False)

    template_prm = package_root/"hamageolib/research/haoyuan_collision0/files/06012026/post_compressible_test.prm"
    template_wb = package_root/"hamageolib/research/haoyuan_collision0/files/06012026/original.wb"
    template_json = fixture_dir/"current.json"
...
def test_reference_case_07032026():
    """
    Test model setup with the reference case setup as of 07032026
    """
    # set up tests
    # case_dir - directory of the test case. Remove the old ones if existing previously
    # template_prm and template_wb - templates to use for the prm and wb file
    # case_prm and case_wb - the prm and wb files generated
    # case_prm_standard and case_wb_standard - the standand output file to compare with
    case_dir = test_dir/"test_reference_case_07032026"
    fixture_dir = fixture_root/"test_reference_case_07032026"

    if case_dir.is_dir():
        shutil.rmtree(case_dir)
    case_dir.mkdir(exist_ok=False)

    template_prm = package_root/"hamageolib/research/haoyuan_collision0/files/07012026/post_compressible_test.prm"
    template_wb = package_root/"hamageolib/research/haoyuan_collision0/files/07012026/original.wb"
    template_json = fixture_dir/"current.json"
...
def test_corner_oceanic_ridges():
    """
    Test model setup with two oceanic ridges on the corner
    """
    # set up tests
    # case_dir - directory of the test case. Remove the old ones if existing previously
    # template_prm and template_wb - templates to use for the prm and wb file
    # case_prm and case_wb - the prm and wb files generated
    # case_prm_standard and case_wb_standard - the standand output file to compare with
    case_dir = test_dir/"test_corner_oceanic_ridges"
    fixture_dir = fixture_root/"test_corner_oceanic_ridges"

    if case_dir.is_dir():
        shutil.rmtree(case_dir)
    case_dir.mkdir(exist_ok=False)

    template_prm = package_root/"hamageolib/research/haoyuan_collision0/files/07172026/post_compressible_test.prm"
    template_wb = package_root/"hamageolib/research/haoyuan_collision0/files/07172026/original.wb"
    template_json = fixture_dir/"current.json"
...
def test_initial_topography_from_file():
    """
    Test model setup with an intial topography
    """
    # set up tests
    # case_dir - directory of the test case. Remove the old ones if existing previously
    # template_prm and template_wb - templates to use for the prm and wb file
    # case_prm and case_wb - the prm and wb files generated
    # case_prm_standard and case_wb_standard - the standand output file to compare with
    case_dir = test_dir/"test_initial_topography_from_file"
    fixture_dir = fixture_root/"test_initial_topography_from_file"
    ...
    case_prm = case_dir/"case.prm"
    case_wb = case_dir/"case.wb"
    initial_topo = package_root/"dtemp"/"new_case_foo"/"inital_topography.txt"

    case_prm_standard = fixture_dir/"case.prm"
    case_wb_standard = fixture_dir/"case.wb"
    initial_topo_standard = fixture_dir/"inital_topography.txt"

    # Remove old initial topography file
    if initial_topo.exists():
        os.remove(initial_topo)
    ...
    assert(filecmp.cmp(case_prm, case_prm_standard))
    assert(filecmp.cmp(case_wb, case_wb_standard))
    assert(filecmp.cmp(initial_topo, initial_topo_standard))